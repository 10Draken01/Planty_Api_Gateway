...
import { ImprovedFitnessCalculator, Objective } from '../../domain/services/ImprovedFitnessCalculator';
 ...
 * 1. Selección inteligente de plantas desde lista del usuario
 * 2. Límite de especies simultáneas (3 o 5)
 * 3. Fitness mejorado con 6 métricas
 * 4. Operadores genéticos avanzados
 */
export class ImprovedGenerateGardenUseCase {
  constructor(
  ...
  async execute(request: GenerateGardenRequestDto): Promise<GenerateGardenResponseDto> {
    try {
      ...
      logger.info('Generando huerto MEJORADO', { request: normalizedRequest });
      ...
      const agConfig: ImprovedGAConfig = {
        ...
        insertionRate: 0.1, // NUEVO: 10% probabilidad de inserción
        deletionRate: 0.05, // NUEVO: 5% probabilidad de eliminación
        ...
        maxSpecies: normalizedRequest.maxPlantSpecies, // NUEVO
      };
      ...
      const constraints: ImprovedGAConstraints = {
        ...
        desiredCategoryDistribution: normalizedRequest.categoryDistribution,
        desiredPlantSpecies: normalizedRequest.desiredPlants, // NUEVO
      };
      ...
      return {
        ...
        metadata: {
          ...
          selectedPlants: result.selectedPlants.map(p => ({ // NUEVO
            species: p.species,
      ...
      logger.error('Error generando huerto MEJORADO', { error: error.message });
      throw error;
    }
  }
  ...
  private normalizeRequest(request: GenerateGardenRequestDto): Required<GenerateGardenRequestDto> {
    ...
    return {
      ...
      maxPlantSpecies: request.maxPlantSpecies || 5, // NUEVO: default 5 especies
